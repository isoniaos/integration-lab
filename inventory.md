...
| `scripts/README.md` | current lab guidance | Validator description covers stale-name rejection, source/trust-boundary requirements, provider-registry checks, and Safe synthetic fixture validation. |
...
| `snapshot/README.md` | current provider note | Includes adapter-spec maturity label, fixture-plan pointer, and evidence needed before a Snapshot integration claim. |
| `snapshot/read-only-offchain-governance-signal-fixture-plan.md` | current provider note | Lab-facing Snapshot fixture plan aligned with the private adapter spec; live/testnet capture remains future evidence work, and no product adapter, vote-submission path, or write path is present. |
| `snapshot/testnet-space-checklist.md` | current provider note | Includes unverified-field-note status, fixture-plan pointer, and required evidence gate. |
