...
| `provider-registry/external-source-provider-registry.example.json` | current lab evidence/template | Conservative machine-readable registry for source categories, record types, authority/evidence posture, visibility/access, freshness/failure concerns, maturity labels, known limits, and next evidence needed. Safe remains `adapter-spec` maturity based on the private spec, fixture plan, synthetic fixture pack, and validator. Snapshot remains `adapter-spec` maturity based on the private spec, lab fixture plan, synthetic fixture pack, and validator. No Safe or Snapshot adapter, write, vote-submission, or execution path is present. |
...
| `scripts/README.md` | current lab guidance | Validator description covers stale-name rejection, source/trust-boundary requirements, provider-registry checks, Safe synthetic fixture validation, and Snapshot synthetic fixture validation. |
...
| `scripts/validate-safe-read-only-execution-fixtures.mjs` | current lab validator | Validates the synthetic Safe read-only execution/evidence fixture pack, authority/evidence posture, mismatch and stale states, no-secret hygiene, shape checks, and no enabled write or execution capabilities. |
| `scripts/validate-snapshot-read-only-offchain-fixtures.mjs` | current lab validator | Validates the synthetic Snapshot read-only offchain governance-signal fixture pack, authority/evidence posture, mismatch, stale, strategy partial, manual link-only, no-secret, shape, and no enabled write/vote/signing/administration/execution capabilities. |
| `snapshot/README.md` | current provider note | Includes adapter-spec maturity label, fixture-plan and synthetic-fixture pointers, and evidence needed before a Snapshot integration claim. |
| `snapshot/read-only-offchain-governance-signal-fixture-plan.md` | current provider note | Lab-facing Snapshot fixture plan synchronized with the synthetic fixture baseline and validator; live/testnet capture remains future evidence work, and no product adapter, vote-submission path, or write path is present. |
| `snapshot/testnet-space-checklist.md` | current provider note | Includes unverified-field-note status, fixture-plan and synthetic-fixture pointers, and required evidence gate. |
| `snapshot/fixtures/README.md` | current lab guidance | Documents the synthetic-only fixture boundary, validator command, and no availability/no vote/no signing/no write boundary. |
| `snapshot/fixtures/read-only-offchain-governance-signal.example.json` | current lab evidence/template | Synthetic Snapshot read-only offchain governance-signal fixture pack covering proposal existence, vote occurrence, discussion context, page/API/cache mismatch, strategy/voting-power partial context, source-unavailable state, and manual link-only evidence. |
...
| `safe/read-only-execution-evidence-fixture-plan.md` | current provider note | Lab-facing Safe fixture plan synchronized with the synthetic fixture baseline and validator; live/testnet capture remains future evidence work, and no product adapter or write path is present. |
| `safe/fixtures/README.md` | current lab guidance | Documents the synthetic-only fixture boundary, validator command, and no support/no write boundary. |
| `safe/fixtures/read-only-execution-evidence.example.json` | current lab evidence/template | Synthetic Safe read-only execution/evidence fixture pack covering executed transaction evidence, bounded payment evidence, mismatch, source-unavailable state, and manual link-only evidence. |
